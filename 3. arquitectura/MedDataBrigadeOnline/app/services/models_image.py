...
from app.core.helper_image import process_image


class HeartbeatClassificationModelImage:
    def __init__(
        self, model_dir,
    ):
    ...
    def predict(self, payload: HeartbeatPayload):
        ...
        start_at = time.time()
        heartbeat_base64 = payload.sequence
        heartbeat_sequence, fs = process_image(heartbeat_base64)  # Procesamos la imagen
        print('fs: ', fs)

        # imprimimos heartbeat_sequence y su tipo
        #print('heartbeat_sequence: ', heartbeat_sequence)
        #print('type heartbeat_sequence: ', type(heartbeat_sequence))
        # extraemos el array de la tupla
        #heartbeat_sequence = heartbeat_sequence[0]
        # convertimos a lista
        heartbeat_sequence = heartbeat_sequence.tolist()
        #print('heartbeat_sequence: ', heartbeat_sequence)
        #print('type heartbeat_sequence: ', type(heartbeat_sequence))
        # Latidos es un dataframe, segmentos es un array
        latidos, segmentos = proceso_segmentacion(heartbeat_sequence, fs) ## Añadimos la función para obtener latidos
